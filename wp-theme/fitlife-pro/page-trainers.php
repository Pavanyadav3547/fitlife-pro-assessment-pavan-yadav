<?php
/**
 * Template Name: FitLife Trainers Directory
 */
...
// Enable Transient caching (Task 5.1 - 12 hours)
$transient_key = 'fitlife_trainers_query_' . ( ! empty( $current_specialty ) ? md5( $current_specialty ) : 'all' );
$trainers_query = get_transient( $transient_key );

if ( false === $trainers_query ) {
    $trainers_query = new WP_Query( $args );
    set_transient( $transient_key, $trainers_query, 12 * HOUR_IN_SECONDS );
}
?>
...
<main id="primary-content" class="container" style="padding-top: var(--space-lg);">
    ...
    <div class="filter-bar">
        <a href="<?php echo esc_url( get_permalink() ); ?>" class="filter-btn <?php echo empty($current_specialty) ? 'active' : ''; ?>">
            <?php esc_html_e( 'All Specialties', 'fitlife' ); ?>
        </a>
        <?php foreach ( $specialties as $specialty ) : ?>
            <?php 
            $filter_url = add_query_arg( 'specialty_filter', $specialty->slug, get_permalink() );
